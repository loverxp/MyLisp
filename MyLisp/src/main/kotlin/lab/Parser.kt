...
//        return parse(ast = expr)
    ...
    private fun parse(ast: Any): Expr {
        ...
        return when (ast) {
            is Int ->
                Number(value = ast.toDouble())
            is Double ->
                Number(value = ast)
            is String ->
                Symbol(symbol = ast)
            is List<*> ->
                parseList(ast = ast as List<Any>)
            else ->
                throw Exception(" TypeNotSupported")
        }
    }
    ...
    private fun parseList(ast: List<Any>): Expr {
        when (ast.size) {
            ...
            3 -> {
                ...
                return when (exp1) {
                    "let" ->
                        parseLet(bindings = exp2 as List<Any>, expr = exp3)
                    "λ", "lambda" ->
                        parseLambda(expr1 = exp2, expr2 = exp3)
                    else ->
                        parseCalc(op = exp1, expr1 = exp2, expr2 = exp3)
                }
            }
            else -> {
